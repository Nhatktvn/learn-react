...
const GalleryItem = () => {
  const { photos, favouriteList, toggleFavourite, addToCart, cartItems } =
    useGallery();
  ...
  return (
    <Fragment>
      <div className="p-5 w-max flex gap-x-3">
        <div className="relative w-max">
          ...
          <span className="absolute w-6 h-6 bg-[#FC2872] text-white rounded-full text-center -top-2 -right-2 z-10">
            {cartItems.length}
          </span>
        </div>
        <div className="relative w-max">
      ...
      <div className="grid grid-cols-4 gap-5">
        {photos.map((item, idx) => {
          return (
            <div className="h-[500px]" key={item.id}>
              <div className="relative h-[500px] group">
                ...
                />
                <button
                  className="absolute top-3 right-3 cursor-pointer opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all"
                ...
                >
                  ...
                  >
                    <path
                      ...
                      fill={item.isFavourite === true ? "red" : "#FFFF"}
                    />
                  </svg>
                </button>
                <button
                  className="absolute p-5 rounded-lg text-2xl font-medium bg-white bottom-[40px] left-2/4 -translate-x-2/4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all "
